...
void check_advise(int row, int column)
{
...
bool check_by_pawns(int start[],int player)
{
	bool danger = false;
	int row = start[0];
	int column = start[1];
...
bool check_by_horse(int start[], int player)
{
	bool danger = false;
	int row = start[0];
	int column = start[1];
	char evil_horse;
	if(player==1){evil_horse = 'h';}
	if(player==2){evil_horse = 'H';}
	
	if(board[row-1][column+2]==evil_horse){check_advise(row-1,column+2);danger = true;}
	if(board[row+1][column+2]==evil_horse){check_advise(row+1,column+2);danger = true;}
	if(board[row-2][column-1]==evil_horse){check_advise(row-2,column-1);danger = true;}
	if(board[row-2][column+1]==evil_horse){check_advise(row-2,column+1);danger = true;}
	if(board[row-1][column-2]==evil_horse){check_advise(row-1,column-2);danger = true;}
	if(board[row+1][column-2]==evil_horse){check_advise(row+1,column-2);danger = true;}
	if(board[row+2][column-1]==evil_horse){check_advise(row+2,column-1);danger = true;}
...
bool warning(int row_,int column_,int player)
{
	...
	///inmediate movement
	if(check_by_pawns(place,player)==true){danger_1 = true;}
	///horse mv
	if(check_by_horse(place,player)==true){danger_1 = true;}
	
	return danger_1;
}

bool check_mate(int player)
{
	char my_king;
	int row_;
	int column_;
	...
	{
		for(int k = 0; k<8;k++)
		{if(board[i][k]==my_king){row_ = i;column_ = k; break;}}
	}
	
	if(warning(row_,column_+1,player)==false){return false;}
	if(warning(row_-1,column_+1,player)==false){return false;}
	if(warning(row_-1,column_,player)==false){return false;}
	if(warning(row_-1,column_-1,player)==false){return false;}
	if(warning(row_,column_-1,player)==false){return false;}
	if(warning(row_+1,column_-1,player)==false){return false;}
	if(warning(row_+1,column_,player)==false){return false;}
	if(warning(row_+1,column_+1,player)==false){return false;}
	cout<<"elhuevomio"<<endl;
return true;
}
